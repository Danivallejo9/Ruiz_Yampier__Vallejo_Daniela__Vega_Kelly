...
def query_orders_per_day_and_holidays_2017(database: Engine) -> QueryResult:
    ...
    holidays["date"] = pd.to_datetime(holidays["date"]).dt.date  # Convertir a tipo datetime
    order_purchase_ammount_per_date["date"] = order_purchase_ammount_per_date["order_purchase_timestamp"]

    result_df = order_purchase_ammount_per_date.merge(
        holidays, on="date", how="left"
    ).fillna({"holiday": False})  # Llenar valores nulos con False en la columna de festivos
